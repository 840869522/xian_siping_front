    ...
    <a-col :push="6" :span="12">
      <a-card :bordered="false">
        <a-form class="permission-form" :form="form" @submit="bindData">
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="原托盘编号：">
            <a-input placeholder="原托盘编号" @keydown.enter="queryData"
              v-decorator="['pallet_code', { rules: [{ required: true, validator, message: '原托盘编号不能为空!' }] }]" />
          </a-form-item>
          ...
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="目标托盘编号：">
            <a-input placeholder="目标托盘编号："
              v-decorator="['pallet_code2', { rules: [{ required: true, validator, message: '目标托盘编号不能为空!' }] }]" />
          </a-form-item>
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="目标暂存区号：">
            <a-input placeholder="目标暂存区号："
              v-decorator="['pallet_code3', { rules: [{ required: true, validator, message: '目标暂存区号不能为空!' }] }]" />
          </a-form-item>
  ...
  methods: {
    ...
    bindData(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          const palletCodeNew = values.pallet_code2
