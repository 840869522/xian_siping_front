  ...
  <a-card :bordered="false">
    ...
    <s-table row-key="login_name" bordered ref="table" size="default" :columns="columns"
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <a @click="updatePsw(record)">修改密码</a>
      </span>
    </s-table>
    <!-- 编辑 -->
    <a-modal title="编辑" style="top: 20px" :width="800" v-model="visibleUpdate" @ok="updateSure">
      <a-form class="permission-form" :form="formUpdate">
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="登录账号">
          <a-input placeholder="登录账号" disabled="disabled"
            v-decorator="['login_name', { rules: [{ required: true, validator, message: '请输入登录账号!' }] }]" />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="用户姓名">
          <a-input placeholder="用户姓名"
            v-decorator="['user_name', { rules: [{ required: true, validator, message: '请输入用户姓名!' }] }]" />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="角色名称">
          <a-select placeholder="请选择角色" showArrow mode="multiple" @change="controlOption"
            v-decorator="['role_name', { rules: [{ required: true, message: '请选择角色名称!' }] }]">
            <a-select-option v-for="item in roleList" :key="item.role_id" :value="item.role_name">
              {{ item.role_name }}
            </a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="账号状态"
          validateStatus="warning">
          <a-select
            v-decorator="['status', { initialValue: 2, rules: [{ required: true, message: '请选择账号状态!' }] }]">
            <a-select-option v-for="item in statusDic" :hidden="item.dic_value == 0"
              :key="item.dic_value" :value="item.dic_value">
              {{ item.dic_display_value }}
            </a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item hidden :labelCol="labelCol" :wrapperCol="wrapperCol" label="更新人">
          <a-input placeholder="更新人" v-decorator="['updater']" />
        </a-form-item>
      </a-form>
    </a-modal>
    <!-- 修改密码 -->
    <a-modal title="修改密码" style="top: 20px" :width="800" v-model="visibleUpdatePsw"
      @ok="updateSurePsw">
      <a-form class="permission-form" :form="formUpdatePsw">
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="登录账号">
          <a-input placeholder="登录账号" disabled="disabled"
            v-decorator="['login_name', { rules: [{ required: true, validator, message: '请输入登录账号!' }] }]" />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="旧密码"
          :validate-status="testStatus" :help="testStatus==='error'?'旧密码错误!':''">
          <a-input @blur="test_old_password" placeholder="旧密码" type="password"
            v-decorator="['old_password', { rules: [{ required: true, validator, message: '旧密码错误!' }] }]" />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="新密码">
          <a-input placeholder="新密码" type="password"
            v-decorator="['new_password', { rules: [{ required: true, validator, message: '请输入新密码!' }] }]" />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="确认新密码"
          :validate-status="testNew" :help="testNew==='error'?'两次输入登录密码不一致!':''">
          <a-input placeholder="确认新密码" type="password"
            v-decorator="['sure_new_password', { rules: [{ required: true, validator, message: '两次输入登录密码不一致!' }] }]" />
        </a-form-item>
        <a-form-item hidden :labelCol="labelCol" :wrapperCol="wrapperCol" label="更新人">
          <a-input placeholder="更新人" v-decorator="['updater']" />
        </a-form-item>
    ...
    <a-modal title="新增" style="top: 20px" :width="800" v-model="visibleAdd" @ok="addDataSure">
      <a-form class="permission-form" :form="formAdd">
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="用户密码">
          <a-input placeholder="用户密码" type="password"
            v-decorator="['password', { rules: [{ required: true, validator, message: '请输入用户密码!' }] }]" />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="用户姓名">
          <a-input placeholder="用户姓名"
            v-decorator="['user_name', { rules: [{ required: true, validator, message: '请输入用户姓名!' }] }]" />
        ...
        <a-form-item hidden :labelCol="labelCol" :wrapperCol="wrapperCol" label="创建人">
          <a-input placeholder="创建人" v-decorator="['creator']" />
        </a-form-item>
      </a-form>
    </a-modal>
  </a-card>
